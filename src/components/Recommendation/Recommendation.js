...
export default function Recommendation({ name, youtubeLink, score, id, onUpvote = () => 0, onDownvote = () => 0 }) {
  ...
  return (
    <Container>
      <Row data-cy="song_name">{name}</Row>
      <ReactPlayer url={youtubeLink} width="100%" height="100%" />
      <Row data-cy="row" >
        <GoArrowUp size="24px" data-cy="handle_upvote" onClick={handleUpvote} />
        {score}
        <GoArrowDown size="24px" data-cy="handle_downvote"  onClick={handleDownvote} />
      </Row>
    </Container>
  );
}
